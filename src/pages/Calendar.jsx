...
export default function Calendar() {
  ...
  const [form, setForm] = useState({ title: '', date: '', time: '', type: 'reminder', color: 'indigo' })
  ...
  function getItemsForDay(day) {
    ...
    return {
      ...
      grades: grades.filter(g => g.created_at?.startsWith(ds)),
  ...
  async function handleAddEvent() {
    ...
    await addEvent(form)
    setShowModal(false)
    setForm({ title: '', date: '', time: '', type: 'reminder', color: 'indigo' })
  }

  function openAddEvent(day) {
    setForm({ title: '', date: day ? dateStr(day) : '', time: '', type: 'reminder', color: 'indigo' })
    setShowModal(true)
  }
        ...
        <div className="flex-1">
                    ...
                    <>
                      ...
                      <div className="space-y-0.5 hidden sm:block">
                        {items.assignments.slice(0, 1).map(a => {
                          ...
                          return (
                            <div key={a.id} className={`text-xs px-1 py-0.5 rounded truncate ${a.status === 'done' ? 'bg-gray-700 text-gray-500' : subject ? subject.color.bg + ' text-white' : 'bg-indigo-600 text-white'}`}>
                              📝 {a.title}
                            </div>
                          )
                        })}
          ...
          <div className="flex gap-4 mt-3 flex-wrap">
            <div className="flex items-center gap-1.5 text-xs text-gray-500">
              <span className="w-2 h-2 rounded-full bg-indigo-500" /> Assignments
            </div>
            <div className="flex items-center gap-1.5 text-xs text-gray-500">
              <span className="w-2 h-2 rounded-full bg-emerald-500" /> Grades
            </div>
            <div className="flex items-center gap-1.5 text-xs text-gray-500">
              <span className="w-2 h-2 rounded-full bg-amber-500" /> Events
            </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
            {selected ? (
                ...
                {selectedItems.assignments.map(a => {
                  ...
                  return (
                    <div key={a.id} className="bg-gray-800 rounded-lg p-3 mb-2">
                      ...
                      {a.due_time && (
                        <p className="text-xs text-indigo-400 mt-0.5">🕐 {formatTime(a.due_time)}</p>
                      )}
                      {subject && (
                  ...
                  return (
                    <div key={e.id} className="bg-gray-800 rounded-lg p-3 mb-2">
                      <div className="flex items-center justify-between">
                        ...
                        <button onClick={() => deleteEvent(e.id)} className="text-gray-600 hover:text-red-400 text-xs transition">✕</button>
                      </div>
                      {e.time && (
                        <p className="text-xs text-indigo-400 mt-0.5">🕐 {formatTime(e.time)}</p>
                      )}
              ...
              <>
                ...
                {[
                  ...assignments
                    .filter(a => a.status !== 'done' && new Date(a.due_date) >= new Date(today.toDateString()))
                ...
                {assignments.filter(a => a.status !== 'done').length === 0 && events.length === 0 && (
              ...
              <div className="flex gap-2">
                {Object.entries(EVENT_COLORS).map(([key, val]) => (
                  <button
                    ...
                    className={`w-7 h-7 rounded-full ${val.bg} ring-2 ring-offset-2 ring-offset-gray-900 transition ${form.color === key ? 'ring-white' : 'ring-transparent'}`}
                  />
                ))}
              </div>
            </div>
